...
class face_rec():
    def __init__(self):
        ...
        self.mask_model = MobileNet(
            input_shape=[self.Crop_HEIGHT, self.Crop_WIDTH, 3], classes=self.NUM_CLASSES)
    ...
    def recognize(self, draw):
        ...
        rectangles_temp[:, [0, 2]] = np.clip(
            rectangles_temp[:, [0, 2]], 0, width)
        rectangles_temp[:, [1, 3]] = np.clip(
            rectangles_temp[:, [1, 3]], 0, height)
        #   对检测到的人脸进行编码
        classes_all = []
        for rectangle in rectangles_temp:
            ...
            crop_img = draw_rgb[int(rectangle[1]):int(
        ...
        for (left, top, right, bottom), c in zip(rectangles, classes_all):
            ...
            cv2.putText(draw, c, (left, bottom - 15),
