...
def generate_arrays_from_file(lines, batch_size, train):
    ...
    while 1:
        ...
        for b in range(batch_size):
            ...
            if train == True:
                img = np.array(get_random_data(
                    img, [HEIGHT, WIDTH]), dtype=np.float64)
            else:
                img = np.array(letterbox_image(
                    img, [WIDTH, HEIGHT]), dtype=np.float64)
        ...
        Y_train = np_utils.to_categorical(
            np.array(Y_train), num_classes=NUM_CLASSES)
...
if __name__ == "__main__":
    ...
    print('freeze the first {} layers of total {} layers.'.format(
        trainable_layer, len(model.layers)))
    ...
    if True:
        ...
        model.fit_generator(generate_arrays_from_file(lines[:num_train], batch_size, True),
                            ...
                            validation_data=generate_arrays_from_file(
                                lines[num_train:], batch_size, False),
